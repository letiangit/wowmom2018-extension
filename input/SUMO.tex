...
\item \label{sm_tr_5} \textcolor{black}{The sampling strategy is applied to select the next 10 sample data points to improve the model accuracy.} 
...
The surrogate model aims to accurately predict \textcolor{black}{performance (e.g., throughput, energy consumption)} for a RAW group $r$ with duration $d_r$, consisting of $s_r$ slots, and with $n_r$ stations assigned to it. The resulting model can be represented as functions $\mathcal{F}$ , as follows:
