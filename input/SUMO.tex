...
The surrogate model aims to accurately predict throughput $t_r$ \textcolor{red}{and energy consumption $e_r$}, for a RAW group $r$ with duration $d_r$, consisting of $s_r$ slots, and with $n_r$ stations assigned to it. The resulting model can be represented as functions $\mathcal{F}_t$ and $\mathcal{F}_e$, as follows:
\begin{equation} \label{eq:sumo_model}
t_r = \mathcal{F}_t(n_r, d_r, s_r) 
\end{equation}
\textcolor{red}{
\begin{equation} \label{eq:sumo_model}
e_r = \mathcal{F}_e(n_r, d_r, s_r) 
\end{equation}
}
