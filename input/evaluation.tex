...
In this section, we analysis the impact of different \gls{raw} configurations on the throughout and energy performance.
...
% \begin{figure}[t]
%     \centering
%     \subfloat[LT, all energy \label{fig:traffic-pattern-throughput}]{\includegraphics[width=0.35\textwidth]{image/pareto-front-energy-865dotsFor401}}\\
%     \subfloat[HT, all energy \label{fig:traffic-pattern-throughput}]{\includegraphics[width=0.35\textwidth]{image/HT-pareto-front-energy}}
%   \caption{\gls{raw} configurations for top 10\% highest energy efficiency. \label{fig:sumo-energy}}
% \end{figure}
...
For the throughput, the small number of stations normally have more near optimal \gls{raw} configurations, which is straightforward, as it needs less time for transmission.

A certain ranges of \gls{raw} slot can leads to good high throughput. For example, for \gls{raw} duration 204.800 ms, slot number [9, 16],  [21, 26] and [40, 50] achieve the highest throughput.

There is a number of \gls{raw} slot numbers that can achieve high performance. For a certain slot number $s_r$ , once a high throughput can be achieved, a similar throughput can also be obtained with slot number $s_r^{'}$ larger than $s_r$. The reason is,
$s_r^{'}$ number leads to less number of stations assigned to one \gls{raw} slot, therefore mitigate the channel contention, and maintain the throughput, or increase the throughput if there are more packets need to be sent. However, there are several exceptions. First of all, if the number of slots is too big, it may cause the \gls{raw} slot duration too short to transmit a single packet. Second,  \todo{the effect of non-cross slot boundary}. While for cross slot boundary, the performance fluctuation
does not occur  \cite{WoWMoM2016} and \cite{Zheng2014}.
Second, it can be observed that the normalized throughput increases in a fluctuating way as the duration of the RAW slot increases. The reason is that a varying number of mini-slots can be wasted due to the RAW slot handover between groups.
