...
\textcolor{red}{The results show, for the throughput models of both high-throughput (HT) and low-throughput (LT) stations, convergence occurs after around $1700$ training samples have been used. The convergence of energy models happens after 1900 and 2500 training samples have been used for energy models of  HT and LT stations, respectively. The training of the HT throughput model and LT energy model stopped after 2370 and 2400 iterations respectively, as it satisfied the cross-validation stop conditions, i.e., 10 consecutive cross-validation scores (2 digits of precision) occurs. The training of the other models stopped after the maximum number of 2500 iterations, having achieved 15 consecutive cross-validation scores between $0.11$ and $0.12$ for LT throughput model, and 9 consecutive cross-validation scores between $0.054$ and $0.058$ for HT energy model.} This comes down to about $0.0074$\% of all data points in the input space (i.e., $2.3 \times 10^7$), and about $6.8$\% of the reduced data space (i.e., $25047$) from which samples were drawn during training. \textcolor{red}{The cross-validation scores decrease as more training samples have been used is due to the sampling strategy FLOLA-Voronoi. It selects points from non-linear regions and the sparsely sampled regions, making the model more and more accurate. It should be noted, the cross-validation scores fluctuate sometimes, that is  because new training samples points are selected from an untouched region, and the surrogate model is  recalculated to consider the new change.}
...
\begin{figure}[t]
    \centering
    \subfloat[Throughput \label{fig:traffic-pattern-throughput}]{\includegraphics[width=0.35\textwidth]{image/LT-pareto-front-throughput}}\\
    \subfloat[Energy consumption per packet \label{fig:traffic-pattern-energy}]{\includegraphics[width=0.35\textwidth]{image/LT-pareto-front-energyPerPacket}}
...
\begin{figure*}[t]
    \centering
    \subfloat[Throughput \label{fig:traffic-pattern-throughput}]{\includegraphics[width=0.75\columnwidth]
    {image/algorithm/throughput-aidaddress-alpha-1-0}} 
    \subfloat[Energy consumption \label{fig:traffic-pattern-throughput}]{\includegraphics[width=0.75\columnwidth]
...
\textcolor{red}{
In this section we evaluate the performance of \gls{moroa} for homogeneous stations (in terms of \gls{mcs} and packet size) with $\alpha = 0$ and $\alpha = 1$ for a variety of traffic loads and station counts. Two different total traffic loads are simulated for the LT scenario, i.e., $T$ = \{0.11, 0.15\}~Mbps.  Given the packet payload size and \gls{mcs}, the maximum throughput that can be achieved is about 0.124~Mbps for LT stations. As such, $T = 0.15$~Mbps represents a near-saturated state, $T = 0.11$~Mbps represents a medium traffic load. Each station randomly chooses its transmission transmission interval, and the ratio between maximal and minimal transmission interval is up to 20 \cite{Sensor2017}. }
As \gls{etaroa} was already shown to outperform \gls{edca} for homogeneous stations, we use \gls{etaroa} as a benchmark in this section~\cite{Sensys2017}.
...
\textcolor{red}{Figure~\ref{fig:sumo-home} depicts performance (i.e., throughput and energy consumption) of \gls{moroa} and \gls{etaroa}. It clearly shows that, compared to \gls{etaroa}, \gls{moroa} can tune the values of $\alpha$ to optimize the required performance objective. For $\alpha = 1$, \gls{moroa} improves the throughput up to 4.4\% under near saturated traffic conditions for any number of stations. Under medium traffic conditions, \gls{moroa} also improves performance for a low number of stations. While in terms of energy consumption, when $\alpha$ equals to $1$, \gls{moroa} consumes more energy than \gls{etaroa} does for both near saturated and medium traffic conditions. Moreover, \gls{etaroa} has the similar power consumption for both traffic load, while the power  consumption of \gls{moroa} increases when the network traffic load changes. Conversely, when $\alpha$ is  $0$,  \gls{moroa} consumes up to 21\% less energy at the cost of slight throughput decrease. The above results reveal that, by using surrogate modeling, stations can be assigned to RAW groups in a more optimal way for the desired performance objective by using the corresponding $\alpha$ value. 
...
\begin{figure*}[t]
    \centering
    \subfloat[Throughput, LT \label{fig:throughput-alpha-0}]{\includegraphics[width=0.35\textwidth]{image/algorithm/Final-throughput-alpha-LT}}
    \subfloat[Throughput, HT \label{fig:delay-alpha-0}]{\includegraphics[width=0.35\textwidth]{image/algorithm/Final-throughput-alpha-HT}} \\
     \subfloat[Energy consumption, LT \label{fig:throughput-alpha-0}]{\includegraphics[width=0.35\textwidth]{image/algorithm/Final-energy-alpha-LT}}
    \subfloat[Energy consumption, HT \label{fig:delay-alpha-0}]{\includegraphics[width=0.35\textwidth]{image/algorithm/Final-energy-alpha-HT}}
  \caption{\textcolor{red}{Performance comparison between \gls{moroa} and \gls{edca}, for a heterogeneous network.} \label{fig:sumo-throughput-csma}}
\end{figure*}
...
\textcolor{red}{ The comparison between \gls{moroa} and \gls{edca} is depicted in figure \ref{fig:sumo-throughput-csma}. With a small number of stations (i.e., 400 stations), \gls{moroa} and \gls{edca} have the same throughput. With more than 100 stations, the throughput of \gls{edca} stagnates to about 0.047~Mbps and 0.21~Mbps for LT and HT stations respectively, as the network becomes saturated. However, for $\alpha = 1$ and $\alpha = 0.75$, \gls{moroa} reduces the number of collisions and results in better scaling behavior and a higher saturation limit. For example, with 1600 stations and $\alpha=0.75$, throughput of \gls{moroa} is about 38\% and 20\% higher for LT and HT stations, respectively. If more focus is put on the throughput objective, with $\alpha=1$, \gls{moroa} achieves about 65\% and 47\% higher throughput for LT and HT stations, respectively. In contrast, when $\alpha = 0$ or $\alpha = 0.5$ , \gls{moroa} obtain less throughout than \gls{edca} does.
In terms of energy consumption, \gls{moroa} always significantly outperforms \gls{edca}. For  1600 stations with $\alpha = 0$ or $\alpha = 0.5$,  \gls{moroa} saves about 96\% and 91\% energy for LT and HT stations, respectively. 
%It should be noted that $\alpha = 0$ and $\alpha = 0.5$ result in the same performance, it is because the objective $Q_t$ and $Q_e$ have different ranges. 
}
