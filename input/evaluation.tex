...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\columnwidth]{image/throughput-cv-t-e}
...
This comes down to about $0.0074$\% of all data points in the input space (i.e., $2.3 \times 10^7$), and about $6.8$\% of the reduced data space (i.e., $25047$) from which samples were drawn during training. \todo{add energy consumption model.}
...
\textcolor{red}{
In order to ensure no over-fitting occurred, the surrogate model also provides accurate results for data points outside the reduced input space (cf. Table~\ref{tab:sumo parameters}), Figure~\ref{fig:sumo-test-data} plots the \gls{cdf} of the relative error (i.e., ratio between absolute error and simulation results). In total, 2070 random data points were generated from all $2.3 \times 10^7$ possible points. It shows 90\% of the test data points has a relative error less than 10\% for both the throughput and energy models of HT and LT stations. These results validate the ability of the surrogate model to estimate \gls{raw} performance accurately, even for points outside of the reduced input data space used for initial training.}
