...
\textcolor{red}{The results show, for the throughput models of both high-throughput (HT) and low-throughput (LT) stations, convergence occurs after around $1700$ training samples have been used. The convergence of energy models happens after 1900 and 2500 training samples have been used for energy models of  HT and LT stations, respectively. The training of the HT throughput model and LT energy model stopped after 2370 and 2400 iterations respectively, as it satisfied the cross-validation stop conditions, i.e., 20 consecutive cross-validation scores lower than or equal to $0.xx$ (2 digits of precision). The training of the other models stopped after the maximum number of 2500 iterations, having achieved 15 consecutive cross-validation scores between $0.11$ and $0.12$ for LT throughput model, and 9 consecutive cross-validation scores between $0.054$ and $0.058$ for HT energy model.} This comes down to about $0.0074$\% of all data points in the input space (i.e., $2.3 \times 10^7$), and about $6.8$\% of the reduced data space (i.e., $25047$) from which samples were drawn during training. \textcolor{red}{The cross-validation scores decrease as more training samples have been used is due to the sampling strategy FLOLA-Voronoi. It selects points from non-linear regions and the sparsely sampled regions, making the model more and more accurate. It should be noted, the cross-validation scores fluctuate sometimes, that is  because new training samples points are selected from an untouched region, and the surrogate model is  recalculated to consider the new change.}
...
\textcolor{red}{Figure~\ref{fig:sumo-home} depicts performance (i.e., throughput and energy consumption) of \gls{moroa} and \gls{etaroa}. It clearly shows that, compared to \gls{etaroa}, \gls{moroa} can tune the values of $\alpha$ to optimize the required performance objective. For $\alpha = 1$, \gls{moroa} improves the throughput under near saturated traffic conditions for any number of stations. Under medium traffic conditions, \gls{moroa} also improves performance for a low number of stations. While in terms of energy consumption, \gls{moroa} consumes more energy than \gls{etaroa} does for both near saturated and medium traffic  conditions. Moreover, \gls{etaroa} has the similar power consumption for both traffic load, while the power  consumption of \gls{moroa} increases when the network traffic load changes. Conversely, when $\alpha$ is  $0$,  \gls{moroa} improves the energy efficiency while results in a bit less throughput. The above results reveal that, by using surrogate modeling, stations can be assigned to RAW groups in a more optimal way for the desired performance objective by using the corresponding $\alpha$ value. 
