...
\begin{figure}[t]
    \centering
    \subfloat[LT \label{fig:traffic-pattern-throughput}]{\includegraphics[width=0.35\textwidth]{image/LT-pareto-front-throughput}}\\
    \subfloat[HT \label{fig:traffic-pattern-throughput}]{\includegraphics[width=0.35\textwidth]{image/HT-pareto-front-throughput}}
  \caption{\gls{raw} configurations for top 10\% packet deliver ratio. \label{fig:ana-throughput}}
\end{figure}
...
\begin{figure}[t]
    \centering
    \subfloat[LT \label{fig:pareto-LT}]{\includegraphics[width=0.35\textwidth]{image/LT-pareto-front201}}\\
    \subfloat[HT \label{fig:pareto-HT}]{\includegraphics[width=0.35\textwidth]{image/HT-pareto-front201}}
    \caption{Pareto front of throughput and energy per station. \label{fig:sumo-pareto}}
\end{figure}

\textcolor{red}{In this section, based on the built model, we analysis the impact of different \gls{raw} configurations on throughout and energy consumption, and the optimal \gls{raw} configuration. For each number of stations, we derive the highest output $t_{max}$ and  $e_{min}$ from the built mode $\mathcal{F}_t\left(\cdot\right)$, and Subsequently, we select the \gls{raw} configurations that lead to near optimal performance, i.e., throughput higher than 90\% of $t_{max}$, or energy consumption lower than  110\% of $e_{min}$. Figure \ref{fig:ana-throughput} and \ref{fig:ana-energy} depict these \gls{raw} configurations for 200, 300 and 400 stations, respectively. As the figures show, less number of stations have more near optimal \gls{raw} configurations, which is straightforward, since the channel contention is less fierce and less transmission time is required. Moreover, the near optimal \gls{raw} slot number is discontinuous distributed. For example, for \gls{raw} duration 204.800 ms and 200 stations, slot number [9, 16],  [21, 26] and [40, 50] achieve the highest throughput. The reason is that a varying number of holding time can be wasted due to the \gls{ncsb} feature \cite{Evgeny2018}. Further more, for small number of stations, for example, quite high performance can already be obtained by without \gls{RAW} (i.e., one \gls{raw} slot), while \gls{raw} is definitely needed for large number of stations. When there is not airtime limit, the packet deliver ratio and energy efficiency overlap, there exists some \gls{raw} configurations that can maximize both objectives simultaneously. Figure \ref{fig:sumo-pareto} plots the Pareto front of throughput and energy consumption per packet for 200 stations. As it shows, the LT stations have only one Pareto front solution, i.e., the objective of maximizing throughput and minimizing energy consumption can be achieved simultaneously. While the HT stations have several Pareto front solutions, different solutions can be chosen based on the preference on each objective.}
