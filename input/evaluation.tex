...
\begin{figure}[t]
  ...
  \caption{\gls{raw} configurations for top 10\% packet deliver ratio. \label{fig:sumo-energy}}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{\gls{raw} configurations for top 10\% highest energy effiency. \label{fig:sumo-energy}}
\end{figure}
\textcolor{red}{
In this section, based on the built model, we analysis the impact of different \gls{raw} configurations on throughout and energy consumption.
First, for a certain number of stations, we derive the highest throughput $t_{max}$ and lowest energy consumption $e_{min}$ from the built mode. Second, we select the \gls{raw} configurations that lead to near optimal performance, i.e., throughput higher than 90\% of $t_{max}$, or energy consumption lower than  110\% of $e_{min}$. Figure xx and xx depict these \gls{raw} configurations for 200, 300 and 400 stations, respectively.
As the figures show, less number of stations have more near optimal \gls{raw} configurations, which is straightforward, since the channel contention is less fierce and less transmission time is required. Moreover, the near optimal \gls{raw} slot number is discontinuous distributed. For example, for \gls{raw} duration 204.800 ms and 200 stations, slot number [9, 16],  [21, 26] and [40, 50] achieve the highest throughput. The reason is that a varying number of holding time can be wasted due to the \gls{ncsb} feature \cite{Evgeny2018}. Further more, for small number of stations, for example, it shows quite high performance can already be obtained by without \gls{RAW}, while \gls{raw} is definitely needed for large number of stations. 
When there is not airtime limit, The packet deliver ratio and energy efficiency overlap, there exists some \gls{raw} configurations that can maximize both objectives simultaneously.}
...
\begin{figure}[t]
    \centering
    \subfloat[LT \label{fig:traffic-pattern-throughput}]{\includegraphics[width=0.35\textwidth]{image/LT-pareto-front201}}\\
    \subfloat[HT \label{fig:traffic-pattern-throughput}]{\includegraphics[width=0.35\textwidth]{image/HT-pareto-front201}}
  \caption{ Pareto front of throughput and energy per station. \label{fig:sumo-energy}}
\end{figure}
