...
\textcolor{black}{Second, by using the built surrogate model for \gls{raw} and multi-objective approach, \gls{moroa} is able to either improve throughput up to 5.3\%,  or improve energy energy efficiency by 22.6\% at only a minor 4.4\% throughput loss, compared to the state of the art \gls{etaroa} algorithm. Most importantly, \gls{moroa} supports traffic-aware \gls{raw} optimization for heterogeneous scenarios with variable \glspl{mcs} and packets size. Its objective weight allows to attain either a 65\% throughput increase, or a 96\% energy saving, or a weighted solution in between.
